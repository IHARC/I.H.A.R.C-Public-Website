...
`SUPABASE_SERVICE_ROLE_KEY`, `PORTAL_INGEST_SECRET`, and `PORTAL_ALERTS_SECRET` are required only on the server/edge function side. Never expose them to the client. `PORTAL_EMAIL_FROM` plus the `PORTAL_SMTP_*` variables configure outbound notification delivery via the relay (used by the `portal-alerts` and `portal-admin-invite` edge functions).
