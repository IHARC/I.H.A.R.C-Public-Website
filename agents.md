...
- Transparency/policies are Supabase-backed. Marketing is read-only on `portal.policies` (RLS: anon only sees `status='published'`). CRUD lives in STEVI.
...
- `/policies` and `/policies/[slug]` pull from `portal.policies` via `src/data/policies.ts` (cached with tag `marketing:policies`) and render sanitized HTML.
...
- Tag-based caching: see `src/lib/cache/tags.ts` and `src/lib/cache/invalidate.ts`. Active tags include `metrics`, `mythEntries`, `pitSummary`, per-PIT `pitCount`, `siteFooter`, and `policies`. New data modules must add tags + invalidation helpers if they mutate anything.
